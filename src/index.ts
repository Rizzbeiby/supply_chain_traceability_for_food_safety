...
export default Server(() => {
  ...
  app.put("/products/:id", (req, res) => {
    ...
    const productOpt = foodProductsStorage.get(productId);
    if ("None" in productOpt) {
      res.status(400).send(`Product with ID=${productId} not found`);
    } else {
      ...
      updatedProduct.history.push(
        `Product updated: ${JSON.stringify(req.body)}`
      );
  ...
  app.post("/products/:id/transfer", (req, res) => {
    ...
    if (!newOwner || !newLocation) {
      return res
        .status(400)
        .json({ error: "New owner and new location are required fields" });
    }
    ...
    updatedProduct.history.push(
      `Ownership transferred to ${newOwner} and moved to ${newLocation}`
    );
  ...
  app.post("/products/:id/inspect", (req, res) => {
    ...
    if (!inspector || !result) {
      return res
        .status(400)
        .json({ error: "Inspector and result are required fields" });
    }
